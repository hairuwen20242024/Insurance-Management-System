package com.insurance.system.motorpolicy.endpoints.v2;
...
//    initialize policyExcel as empty file
  ...
  public ResponseEntity<?> listRenewals(@PathVariable Long id, ModelMap model, @SortDefault({"id"}) Pageable pageable) {
    List<MotorPolicyRenewal> data = this.motorPolicyRepository.findById(id).get().getMotorPolicyRenewals();

//    convert objects to page
    Page<MotorPolicyRenewal> objects = new PageImpl<MotorPolicyRenewal>(data);
